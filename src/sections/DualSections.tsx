...
import { FlexCard, MotionElement, ShortInfoCard, type ShortInfoCardData, type MdCardProps } from "@/comps"
import { MarkdownWrapper, SECTION_TYPE, type ComponentClassNames } from "@/layouts"
...
export const DualSections = ({ sections }: DualSectionsData) => (
	<>
			...
			if (section?.type === SECTION_TYPE.FLEX_SECTION) {
				...
				return (
					<div className={twMerge("w-full", section?.body?.classNames?.sectionClassName)} key={section?.type + idx}>
						<FlexCard flexDirection='flex-row-reverse' imgProps={section.body.imgProps} className='max-w-[1920px] p-4'>
