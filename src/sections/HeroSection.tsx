...
import { MotionElement, MotionImage, UnderlinedBtn } from "@/comps"
...
export const HeroSection = ({ imgProps, content, buttons, label }: HeroSectionData) => (
	<>
		{/* Inner Wrapper  */}
		<div className='relative z-[1] mx-auto block w-full max-w-[650px] gap-8 lg:grid lg:max-w-none lg:grid-cols-2 lg:gap-x-10 lg:gap-y-2'>
			...
			<div className='mb-6 h-max max-w-[576px] self-center xs:mb-12 lg:mb-0'>
				...
				<MotionElement className='mt-8 flex flex-col items-center gap-6 xs:flex-row' {...slideRightBtm}>
					<Link
						href={buttons[0]?.href}
						className='group relative inline-block w-full transition-transform duration-300 will-change-transform focus:outline-none active:scale-[99%] xs:w-max'>
						<span className='gradient-primary-light absolute inset-0 translate-x-1.5 translate-y-1.5 rounded-[4px] bg-gradient-to-r transition-transform duration-300 group-hover:translate-x-0 group-hover:translate-y-0'></span>
